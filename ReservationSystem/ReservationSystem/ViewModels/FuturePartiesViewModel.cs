...
namespace ReservationSystem.ViewModels
{
    public class FuturePartiesViewModel : BaseViewModel
    {
        ...
        private List<Suggestion> getSugesstions()
        {
            ...
            {
                return db.Suggestions.Include("PartyRequest").Where(suggestion => suggestion.Answered == AnsweredType.Prihvacen && suggestion.PartyRequest.CreatorId ==  User.Id && suggestion.PartyRequest.Date.CompareTo(DateTime.Now) > 0).OrderBy(suggestion => suggestion.PartyRequest.Date).ToList();
            }
        }
